...
$APPLICATION->SetTitle('Редактирование упражнения');
...
//foreach(\LearningDatabase\ORM\UserExerciseTable::getEntity()->getFields() as $obTestField){
	...
	$arFields[] = [
		...
		'value' => isset($_REQUEST['data'][$fieldName]) ? $_REQUEST['data'][$fieldName] : $arElement[$fieldName],
	];
}
...
if (($_REQUEST['save'] || $_REQUEST['apply']) && check_bitrix_sessid()){
	...
	$result = Exercise::update($arElement['ID'], $arData);
	if($result->isSuccess())
		if($_REQUEST['apply'])
			$arMessages['notes'][] = 'Упражнение успешно обновлёно';
		else
...
if($bShowForm):
	...
	$tabControl = new CAdminTabControl('tabControl', [
		['DIV' => 'edit1', 'TAB' => 'Редактирование', 'TITLE'=> 'Редактирование "'.$arElement['NAME'].'"']
	]);
	?>
